...
//  CDAKit
//
//  Created by Grant Elliott on 7/23/18.
//  Copyright © 2018 Eric Whitley. All rights reserved.
//

import Foundation

/// A thread-safe array.
public class SynchronizedArray<Element: Hashable> {
...
public extension SynchronizedArray {
    ...
    var first: Element? {
        var result: Element?
        self.queue.sync { result = self.array.first }
        return result
    }
    ...
    var last: Element? {
        var result: Element?
        self.queue.sync { result = self.array.last }
        return result
    }
    ...
    var isEmpty: Bool {
        var result = false
        self.queue.sync { result = self.array.isEmpty }
        return result
    }
    ...
    var description: String {
        var result = ""
        self.queue.sync { result = self.array.description }
        return result
    }
}
...
public extension SynchronizedArray {
    ...
    func filter(_ isIncluded: (Element) -> Bool) -> [Element] {
        var result = [Element]()
        self.queue.sync { result = self.array.filter(isIncluded) }
        return result
    }
    ...
    func index(where predicate: (Element) -> Bool) -> Int? {
        var result: Int?
        self.queue.sync { result = self.array.index(where: predicate) }
        return result
    }
    ...
    func sorted(by areInIncreasingOrder: (Element, Element) -> Bool) -> [Element] {
        var result = [Element]()
        self.queue.sync { result = self.array.sorted(by: areInIncreasingOrder) }
        return result
    }
    ...
    func flatMap<ElementOfResult>(_ transform: (Element) -> ElementOfResult?) -> [ElementOfResult] {
        var result = [ElementOfResult]()
        self.queue.sync { result = self.array.flatMap(transform) }
        return result
    }
    ...
    func contains(where predicate: (Element) -> Bool) -> Bool {
        var result = false
        self.queue.sync { result = self.array.contains(where: predicate) }
        return result
    }
}
