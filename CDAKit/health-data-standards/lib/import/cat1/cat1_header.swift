...
class CDAKImport_cat1_HeaderImporter {
  ...
  class func get_authors(_ elem: XMLElement) -> CDAKQRDAAuthor {
    ...
    if let assignedAuthor = elem.xpath("./cda:assignedAuthor").first {
      ...
      author.addresses = assignedAuthor.xpath("./cda:addr").compactMap({addr in CDAKImport_CDA_LocatableImportUtils.import_address(addr)})
      author.telecoms = assignedAuthor.xpath("./cda:telecom").compactMap({tele in CDAKImport_CDA_LocatableImportUtils.import_telecom(tele)})
  ...
  class func set_legal_authenticator(_ header: CDAKQRDAHeader, doc: XMLDocument) {
    if let auth_info = doc.xpath("/cda:ClinicalDocument/cda:legalAuthenticator").first {
      ...
      if let assignedEntity = auth_info.xpath("./cda:assignedEntity").first {
        ...
        legal.addresses = assignedEntity.xpath("./cda:addr").compactMap({addr in CDAKImport_CDA_LocatableImportUtils.import_address(addr)})
        legal.telecoms = assignedEntity.xpath("./cda:telecom").compactMap({tele in CDAKImport_CDA_LocatableImportUtils.import_telecom(tele)})
