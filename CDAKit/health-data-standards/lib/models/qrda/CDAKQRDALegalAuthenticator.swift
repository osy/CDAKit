...
extension CDAKQRDALegalAuthenticator: CustomStringConvertible {
  public var description: String {
    return "CDAKQRDALegalAuthenticator => time:\(time), ids:\(ids), addresses:\(addresses), telecoms:\(telecoms), person:\(person?.description ?? ""), organization: \(organization?.description ?? ""))"
  }
}
