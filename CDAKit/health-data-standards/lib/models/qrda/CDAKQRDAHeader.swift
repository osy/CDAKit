...
extension CDAKQRDAHeader: CustomStringConvertible {
  public var description: String {
    return "CDAKQRDAHeader => identifier:\(identifier?.description ?? ""), authors:\(authors), custodian:\(custodian?.description ?? ""), legal_authenticator:\(legal_authenticator?.description ?? ""), performers:\(performers), time:\(time), confidentiality: \(confidentiality)"
  }
}
